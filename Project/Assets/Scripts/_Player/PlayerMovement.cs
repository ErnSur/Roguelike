...
public class PlayerMovement : MonoBehaviour
{
    ...
    [FormerlySerializedAs("orginalStepVolume")]
    [Range(0, 1)]
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.D) && transform.position == PlayerStats.instance.Position)
        {
            ...
            if (PFgrid.grid[(int) PlayerStats.instance.Position.x + 1, (int) PlayerStats.instance.Position.y]
                    .Walkable == true)
            {
        ...
        else if (Input.GetKey(KeyCode.W) && transform.position == PlayerStats.instance.Position)
        {
            ...
            if (PFgrid.grid[(int) PlayerStats.instance.Position.x, (int) PlayerStats.instance.Position.y + 1]
                    .Walkable == true)
            {
        ...
        transform.position =
            Vector3.MoveTowards(transform.position, PlayerStats.instance.Position, Time.deltaTime * speed);
    ...
    {
        RaycastHit2D hit =
            Physics2D.Raycast(PlayerStats.instance.Position, rayDirection, raycastDistance, statWallLayer);
    ...
    IEnumerator FootstepSound()
    {
        if (foot == 1)
        {
            ...
            audioSrc.PlayOneShot(stepSoundOrginal, stepVolume);
